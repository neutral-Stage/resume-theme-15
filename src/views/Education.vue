<template>
    <v-app>
        <v-row no-gutters justify="center">
            <v-col md="10">
                <v-row>
                    <v-col md="6" sm="12" cols="12">
                        <v-row>
                            <v-col md="3" sm="5" cols="5" class="pr-4">
                                <h1 class="textcol subtitle-1">Master in Web Developer</h1>
                                <h1 class="textcol subtitle-2 mt-12">2015 - 2016</h1>
                            </v-col>
                            <v-col md="9" sm="7" cols="7">
                                <h1 class="textcol headline pb-2">Envision Media, Inc., Santa Cruz, CA</h1>
                                <ul class="textcol body-2">
                                    <li>- Turning creative concepts into finished websites.</li>
                                    <li>- Drawing up detailed website s pecifications.</li>
                                    <li>- Building websites that are easy to understand, navigate and use.</li>
                                    <li>- Designing sample page layouts.</li>
                                    <li>- Ensuring that the website complies with the company’s brand guidelines.</li>
                                </ul>
                            </v-col>
                        </v-row>
                    </v-col>
                    <v-col md="6" sm="12" cols="12">
                        <v-row>
                            <v-col md="3" sm="5" cols="5" class="pr-4">
                                <h1 class="textcol subtitle-1">Master in Web Developer</h1>
                                <h1 class="textcol subtitle-2 mt-12">2015 - 2016</h1>
                            </v-col>
                            <v-col md="9" sm="7" cols="7">
                                <h1 class="textcol headline pb-2">Envision Media, Inc., Santa Cruz, CA</h1>
                                <ul class="textcol body-2">
                                    <li>- Turning creative concepts into finished websites.</li>
                                    <li>- Drawing up detailed website s pecifications.</li>
                                    <li>- Building websites that are easy to understand, navigate and use.</li>
                                    <li>- Designing sample page layouts.</li>
                                    <li>- Ensuring that the website complies with the company’s brand guidelines.</li>
                                </ul>
                            </v-col>
                        </v-row>
                    </v-col>

                </v-row>
                <v-row class="mt-8">
                    <v-col md="6" sm="12" cols="12">
                        <v-row>
                            <v-col md="3" sm="5" cols="5" class="pr-4">
                                <h1 class="textcol subtitle-1">Master in Web Developer</h1>
                                <h1 class="textcol subtitle-2 mt-12">2015 - 2016</h1>
                            </v-col>
                            <v-col md="9" sm="7" cols="7">
                                <h1 class="textcol headline pb-2">Envision Media, Inc., Santa Cruz, CA</h1>
                                <ul class="textcol body-2">
                                    <li>- Turning creative concepts into finished websites.</li>
                                    <li>- Drawing up detailed website s pecifications.</li>
                                    <li>- Building websites that are easy to understand, navigate and use.</li>
                                    <li>- Designing sample page layouts.</li>
                                    <li>- Ensuring that the website complies with the company’s brand guidelines.</li>
                                </ul>
                            </v-col>
                        </v-row>
                    </v-col>
                    <v-col md="6" sm="12" cols="12">
                        <v-row>
                            <v-col md="3" sm="5" cols="5" class="pr-4">
                                <h1 class="textcol subtitle-1">Master in Web Developer</h1>
                                <h1 class="textcol subtitle-2 mt-12">2015 - 2016</h1>
                            </v-col>
                            <v-col md="9" sm="7" cols="7">
                                <h1 class="textcol headline pb-2">Envision Media, Inc., Santa Cruz, CA</h1>
                                <ul class="textcol body-2">
                                    <li>- Turning creative concepts into finished websites.</li>
                                    <li>- Drawing up detailed website s pecifications.</li>
                                    <li>- Building websites that are easy to understand, navigate and use.</li>
                                    <li>- Designing sample page layouts.</li>
                                    <li>- Ensuring that the website complies with the company’s brand guidelines.</li>
                                </ul>
                            </v-col>
                        </v-row>
                    </v-col>
                </v-row>
            </v-col>
        </v-row>

    </v-app>
</template>

<script>
    export default {
        name: "education"
    }
</script>

<style scoped>
    .v-application ol, .v-application ul {
        padding-left: 0px !important;
    }
    ul li{
        list-style: none;
    }
    .large_dot:before {
        bottom: 0;
        content: "";
        height: 0px;
        position: absolute;
        top: 0px;
        width: 0px;
    }
    .theme--light.v-application {
        background: transparent;
        color: rgba(0,0,0,.87);
    }
    .textcol {
        color: #104efb !important;
    }
</style>
